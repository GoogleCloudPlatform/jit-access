...
public class UserContext implements Subject, CatalogUserContext {
  ...
  private final @NotNull Set<GroupEmail> groups;
  ...
  UserContext(
    ...
    this.allPrincipals = Stream
      .concat(
        groups.stream().map(g -> (PrincipalIdentifier)g),
        Stream.of(id))
  ...
  public static class Resolver {
    ...
    public UserContext lookup(
      ...
      var jitRoleGroupMemberships = allMemberships
        .stream()
        .filter(m -> this.mapper.isJitRole(new GroupEmail(m.getGroup())))
        .toList();

      var otherGroups = allMemberships
        .stream()
        .filter(m -> !this.mapper.isJitRole(new GroupEmail(m.getGroup())))
        .map(m -> new GroupEmail(m.getGroup()))
        .collect(Collectors.toSet());
      ...
      for (var future : membershipFutures) {
        try {
          ...
          var groupEmail = new GroupEmail(membership.getPreferredMemberKey().getId());
