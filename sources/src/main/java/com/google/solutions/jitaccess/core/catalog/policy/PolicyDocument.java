...
public class PolicyDocument {
  ...
  record AccessControlListEntryNode(
    ...
    private PrincipalIdentifier principal() {
      if (this.principalString.startsWith(UserEmail.TYPE + ":")) {
        return new UserEmail(this.principalString.substring(UserEmail.TYPE.length() + 1).trim());
      }
      else if (this.principalString.startsWith(GroupEmail.TYPE + ":")) {
        return new GroupEmail(this.principalString.substring(GroupEmail.TYPE.length() + 1).trim());
      }
      else {
        //
