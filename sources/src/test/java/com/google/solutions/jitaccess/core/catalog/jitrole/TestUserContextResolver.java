...
public class TestUserContextResolver {
  ...
  private static final GroupEmail SAMPLE_JITGROUP_1 = new GroupEmail("jit-role-1@example.com");
  private static final GroupEmail SAMPLE_JITGROUP_2 = new GroupEmail("jit-role-2@example.com");
  ...
  private class SampleGroupMapping implements JitRoleGroupMapping {
    @Override
    public boolean isJitRole(GroupEmail group) {
      return group.email.startsWith("jit-");
    }

    @Override
    public JitRole jitRoleFromGroup(GroupEmail email) {
      return new JitRole(
        "policy",
    ...
    @Override
    public GroupEmail groupFromJitRole(JitRole role) {
      return new GroupEmail(String.format("jit-%s@example.com", role.name()));
    }
  }
